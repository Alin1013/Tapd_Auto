...
- 任务归属字段默认 `owner`。
- 缺陷未解决和当日关闭默认按处理人字段 `current_owner`，今日新增默认按创建人字段 `reporter`。
- 产品经理需求字段默认 `owner`。
...
- 每个成员名称下方展示角色和 TAPD user 预览。
- 每个成员的缺陷区域展示未解决、今日新增、当日关闭，并按当天创建的新增缺陷输出 TAPD 状态分色柱状图。
- 当前显式迭代范围内的成员即使当天工作量为 0 也展示为 0。
