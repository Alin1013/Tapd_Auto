...
- `Tora`（黄寅子）和 `nianqiongyue`（粘琼月）不进入缺陷成员表，二者负责的需求合并进入同一块“产品总需求”。
...
- 迭代必须有当日动作才进入日报：至少有今日新增缺陷、当日关闭缺陷，或当天创建/修改/完成/排期覆盖当天的“产品总需求”。
...
- 团队成员默认展示未解决、今日新增、当日关闭缺陷；配置 `hide_bug_metrics: true` 的成员不展示缺陷数字，其产品需求按规则进入“产品总需求”。
