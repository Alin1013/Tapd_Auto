...
5. 按人员聚合今日缺陷，日报展示未解决、今日新增、当日关闭；“今日新增”和状态柱状图均按当天 `created` 的缺陷统计，任务数据保留采集但页面和通知不展示任务数、完成率或进展。
6. 没有显式迭代范围时，仅保留今天有新增/关闭缺陷或当天产品需求动作的项目迭代；配置了 `report_scope.iteration_names` 或 `iteration_ids` 时，即使当天工作量为 0 也展示当前迭代和范围内成员。
